...
df <- df |> 
  ...
  mutate(died_12h = ifelse(index > (index_max - 12) & Died == 1, 1, 0),
...
survset <- df |>
  ...
  arrange(ID, index) |>
  slice_tail(n = 1) |>
  mutate(Outcome = case_when(outcome == 0 & is.end == T ~ factor(0, labels = "Discharged alive"),
                             outcome == 1 & is.end == T ~ factor(1, labels = "Died in hospital"),
                             is.end == F ~ factor(2, labels = "Censored")),
         Days = ceiling(index2/24))

cif.fit <- with(survset, cuminc(ftime = Days, 
                                fstatus = Outcome, 
                                group = Facility, 
                                cencode = "Censored"))

g <- ggcompetingrisks(fit = cif.fit, 
                      conf.int = T,
                      xlab = "Time in days",
                      title = "",
                      legend.title = "")
