...
#Convert consciousness stat to categorical prior to imputation
df <- df |> 
  mutate(AVPU = case_when(Alert == 1 ~ 1,
                          Verbal == 1 ~ 2,
                          Rousable == 1 ~ 3,
                          Unresponsive == 1 ~ 4))
...
#Refix the MAP variable now that SBP/DBP are fully imputed
df$Mean_AP_Cuff_Calc <- df$DBP + (df$SBP - df$DBP)*(1/3)

#Save to disc
export <- "insert custom filepath"
write_parquet(df_mr, paste0(export,"df_mr.parquet"))
